...
ALL_CONTAINERS = ["work-eam", "personal"]
...
class ContainerTest:
    ...
    def verify_common(self) -> None:
        ...
        self.assert_exec(
            ...
            'export NVM_DIR=${NVM_DIR:-$HOME/.config/nvm} && . $NVM_DIR/nvm.sh && node --version',
        )
        self.assert_exec(
            "npx available",
            'export NVM_DIR=${NVM_DIR:-$HOME/.config/nvm} && . $NVM_DIR/nvm.sh && npx --version',
        )
        self.assert_exec("fzf installed", "test -d $HOME/.fzf")
        self.assert_exec("starship installed", "command -v starship")
        self.assert_exec("oh-my-zsh installed", "test -d $HOME/.oh-my-zsh")
        self.assert_symlink("claude config CLAUDE.md", "$HOME/.claude/CLAUDE.md")
        self.assert_file("claude config settings.json", "$HOME/.claude/settings.json")
        self.assert_contains("BROWSER forwarding", "$HOME/.zshrc", "BROWSER")
    ...
    def verify_personal_project(self) -> None:
        console.print("  --- personal-<project>-specific assertions ---")
        self.assert_executable("setup-creds is executable", "$HOME/.local/bin/setup-creds")
        self.assert_exec("glab installed", "command -v glab")
        self.assert_file("atuin installed", "$HOME/.atuin/bin/atuin")
        self.assert_file("atuin config exists", "$HOME/.config/atuin/config.toml")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Test all containers (personal, work-eam)",
    )
    parser.add_argument(
        "--keep",
        action="store_true",
