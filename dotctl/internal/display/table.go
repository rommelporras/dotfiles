...
func RenderAll(machines []model.MachineState, containers []model.ContainerInfo) string {
	...
	if len(containers) > 0 {
		sb.WriteString("\n")
		sb.WriteString(RenderContainers(containers))
	}
	return sb.String()
}
...
// RenderContainers renders the distrobox container status table.
func RenderContainers(containers []model.ContainerInfo) string {
	var sb strings.Builder
	sb.WriteString(headerStyle.Render(" Containers"))
	sb.WriteString("\n")
	sb.WriteString(" " + col("", 24) + " " + col("Status", 12) + " " + col("Image", 20) + "\n")

	for _, c := range containers {
		statusStr := styleValue(c.Status, "running", "stopped")
		sb.WriteString(" " + col(c.Name, 24) + " " + col(statusStr, 12) + " " + dimStyle.Render(c.Image) + "\n")
	}
	return sb.String()
}
