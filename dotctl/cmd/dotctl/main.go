...
func runCollect(cfg *config.Config, container string, verbose bool) error {
	...
	// Push container running status
	if len(result.Containers) > 0 {
		if err := push.PushContainers(ctx, cfg.OTelEndpoint, cfg.Hostname, result.Containers); err != nil {
			if verbose {
				fmt.Fprintf(os.Stderr, "Warning: failed to push container metrics: %v\n", err)
			}
		} else if verbose {
			fmt.Printf("  ✓ container metrics pushed (%d containers)\n", len(result.Containers))
		}
	}
